...
aioSay('\n\tTrying to move from <<toString(rm0.name)>> to <<toString(rm1.name)>>\n ');
...
DefineIAction(Rezrov)
	execAction() {
		...
		pathfinder.updatePathfinder(westDoor);
		pathfinder.updatePathfinder(eastDoor);
